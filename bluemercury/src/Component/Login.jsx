...
export const Login = () => {
    ...
    const handleLogin = async (e) => {
        ...
        let uniqData = usersData.filter(item => item.email === userData.email && item.password === userData.password);
        ...
        <Box w={['280px', '460px']} m='auto' mt='65px' mb='140px' zIndex={1}>
            ...
            <form onSubmit={handleLogin}>
                <Box border='1px solid black' mt='20px' p='7px 14px' _hover={{ borderColor: "blue" }}>
                    <Input variant='unstyled' placeholder='Email' borderRadius='0px' onChange={(e) => setUserData({ ...userData, email: e.target.value })} required/>
                </Box>
                <Box mt='15px' p=' 7px 14px' display='flex' border='1px solid black' _hover={{ borderColor: "blue" }}>
                    <Input placeholder='Password' variant='unstyled' type='password' borderRadius='0px' required onChange={(e) => setUserData({ ...userData, password: e.target.value })} />
